...
bool DFS(int f[][COL], bool r[][10], bool c[][10], bool b[][10]){
    for(int i=0; i<9; i++)
        for(int j=0; j<9; j++)
            if(f[i][j] == 0){
                int k = blockID(i,j);
                for(int n=1; n<=9; n++){    //try the 1~9
                    if(!r[i][n] && !c[j][n] && !b[k][n]){
                        f[i][j] = n;
                        r[i][n] = c[j][n] = b[k][n] = 1;
                        if(DFS(f, r, c, b)) return true;
                        f[i][j] = 0;    //backtrack
                        r[i][n] = c[j][n] = b[k][n] = 0;
                    }
                }
...
bool LasVegas(int n, int field[][COL]){   //拉斯维加斯算法
    ...
    while(n > 0){   //fill n digits randomly
...
bool LasVegas2(int n, int field[][COL]){   //拉斯维加斯算法
    srand((unsigned)time(nullptr));
    bool row[ROW][10], col[COL][10], block[9][10];
    memset(row, 0, sizeof(row));
    memset(col, 0, sizeof(col));
    memset(block, 0, sizeof(block));
    for(int i=0; i<3; i++)  //memset
        for(int j=0; j<3; j++){
            int t = field[i][j];
            int k = blockID(i,j);
...
bool CheckUnique(int x, int y, int f0[][COL], bool r0[][10], bool c0[][10], bool b0[][10]){
    ...
    for(int n=1; n<=9; n++){    //try the 1~9
        if(!r[x][n] && !c[y][n] && !b[k][n] && n!=target){
            f[x][y] = n;
            r[x][n] = c[y][n] = b[k][n] = 1;
            if(DFS(f, r, c, b)) return false;   //found another answer, so not unique
            f[x][y] = 0;    //backtrack
            r[x][n] = c[y][n] = b[k][n] = 0;
        }
    }
...
void Dig(int n, int f1[][COL], int f2[][COL]){
    ...
    while(n){
        ...
        int flag = 1;
        if(id == 1){
            if(k == 8) flag = CheckUnique(x,y,f1,r1,c1,b1) && CheckUnique(x-6,y-6,f2,r2,c2,b2);
            else flag = CheckUnique(x,y,f1,r1,c1,b1);
            if(flag){
                int t = f1[x][y];
                f1[x][y] = 0;
            ...
            if(k == 0) flag = CheckUnique(x+6,y+6,f1,r1,c1,b1) && CheckUnique(x,y,f2,r2,c2,b2);
            else flag = CheckUnique(x,y,f2,r2,c2,b2);
            if(flag){
                int t = f2[x][y];
                f2[x][y] = 0;
...
void MakeSudokuCNF(int f1[][COL], int f2[][COL]){
    ...
    for(int i=0; i<9; i++)
        for(int j=0; j<9; j++){
            if(f1[i][j]) singleClauseNums++;
            if(f2[i][j]) singleClauseNums++;
        }
...
void BasicClause(FILE* fp, int id){
    ...
    for(int i=0; i<9; i++)  
        for(int j=0; j<9; j++){
            ...
            // for(int n=1; n<=9; n++){    //f2[i][j] = n;
            //     int var = VAR(1, i, j, n);
            //     fprintf(fp, "%d  ", var);
            // }   fprintf(fp, "0\n");
            // for(int n=1; n<9; n++){
            //     for(int m=n+1; m<=9; m++){
            //         int var1 = VAR(1, i, j, n), var2 = VAR(1, i, j, m);
            //         fprintf(fp, "%d %d 0\n", -var1, -var2);
            //     }
            // }
        }
    ...
    for(int i=0; i<9; i++){     //row
        for(int n=1; n<=9; n++){
            ...
            // for(int j=0; j<9; j++){ //f2
            //     int var = VAR(1, i, j, n);
            //     fprintf(fp, "%d  ", var);
            // }   fprintf(fp, "0\n");
            // for(int j1=0; j1<8; j1++){
            //     for(int j2=j1+1; j2<9; j2++){
            //         int var1 = VAR(1, i, j1, n), var2 = VAR(1, i, j2, n);
            //         fprintf(fp, "%d %d 0\n", -var1, -var2);
            //     }
            // }
        }
    ...
    for(int j=0; j<9; j++){     //col
        for(int n=1; n<=9; n++){
            ...
            // for(int i=0; i<9; i++){ //f2
            //     int var = VAR(1, i, j, n);
            //     fprintf(fp, "%d  ", var);
            // }   fprintf(fp, "0\n");
            // for(int i1=0; i1<8; i1++){
            //     for(int i2=i1+1; i2<9; i2++){
            //         int var1 = VAR(1, i1, j, n), var2 = VAR(1, i2, j, n);
            //         fprintf(fp, "%d %d 0\n", -var1, -var2);
            //     }
            // }
        }
