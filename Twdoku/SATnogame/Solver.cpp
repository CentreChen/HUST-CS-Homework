...
status DPLL(CNF* cnf, int *result){
    ...
    if(cnf->ClauseNum == 0) return TRUE;

    HeadNode* singleClause_Pre = FindSingleClause_Pre(cnf);
    while(singleClause_Pre != nullptr){ //单规则
        int single = singleClause_Pre->next->head->next->var;
        if(single > 0) result[single] = 1;
        else if(single < 0) result[-single] = 0;
        RemoveClause_Suc(singleClause_Pre, cnf);
        Simplify(single, cnf);
        if(cnf->root->next == nullptr) return TRUE; //no Clause
        else if(FindEmptyClause(cnf)) return FALSE;  //Empty Clause can not be satisfied
        singleClause_Pre = FindSingleClause_Pre(cnf);
    }

    int target = StrategyMax2(cnf);
...
int StrategyMax2(CNF* cnf){
    ...
    free(Num);
    if(pMaxNum==-1 || pMaxNum==0) pMaxNum = StrategyFirst(cnf);
    else if(pMaxNum > VarNum) pMaxNum = -(pMaxNum-VarNum);
    return pMaxNum;
}

int StrategyFirst(CNF* cnf){
...
void Simplify(int target, CNF* cnf){  //simplify cnf with single Clause rule
    HeadNode* pHead = cnf->root;
...
void RemoveClause_Suc(HeadNode* pHead, CNF* cnf){    //remove the successor Node of pHead
    cnf->ClauseNum--;
...
void ReleaseLine(HeadNode* pHead){  //删除该行
    LinkNode* pLink = pHead->head;
    while(pLink->next){
        LinkNode* t = pLink;
        pLink = pLink->next;
